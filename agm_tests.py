...
from logic import Formula, Neg, equivalent, parse, to_clauses, to_cnf
...
def _same_theory(left: list[Formula], right: list[Formula]) -> bool:
    # Cn(left) == Cn(right) via mutual entailment
    return all(entails(right, f) for f in left) and all(entails(left, f) for f in right)


# AGM revision postulates (K*2 - K*6). K*1 (closure) is for belief sets, not bases.
...
class ContractionPostulates(unittest.TestCase):
    ...
    def test_tautology_is_identity(self):
        ...
        self.assertEqual(contraction(self.bb, parse("p | ~p")).items(), self.bb.items())

    def test_recovery_does_not_hold_on_bases(self):
        # K-5 (Recovery) fails for partial-meet on a belief base.
        # B = {p & q}; B / p = {} since {p & q} entails p, so (B/p) + p = {p}
        # does not recover p & q.
        bb = BeliefBase()
        bb.add(parse("p & q"), 1)
        contracted = contraction(bb, parse("p")).formulas()
        self.assertFalse(entails(contracted + [parse("p")], parse("p & q")))


class PartialMeetBehaviour(unittest.TestCase):
    def test_intersection_is_not_maxichoice(self):
        # four equally-preferred remainders -> intersection is empty
        bb = BeliefBase()
        for f in ("p", "p >> r", "q", "q >> r"):
            bb.add(parse(f), 1)
        self.assertEqual(contraction(bb, parse("r")).formulas(), [])

    def test_priority_breaks_ties(self):
        # higher-priority p survives over lower-priority alternatives
        bb = BeliefBase()
        bb.add(parse("p"), 5)
        bb.add(parse("p >> r"), 1)
        bb.add(parse("q"), 1)
